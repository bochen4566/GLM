...
class ActionParser:
    ...
    new_action_space_pattern = {
        ...
        'Select': r'(?s).*?select\([\'\"]([A-Za-z0-9-_]+)[\'\"],\s*[\'\"](.+)[\'\"](?:\s*\)|\s*,\s*.*\))',
        'Go_forward': r'(?s).*?go\([\'\"]forward[\'\"](?:\s*\)|\s*,\s*.*\))',
        'Go_backward': r'(?s).*?go\([\'\"]backward[\'\"](?:\s*\)|\s*,\s*.*\))',
        'Jump_to': r'(?s).*?jump_to\([\'\"](.+?)[\'\"],\s*(True|False)(?:\s*\)|\s*,\s*.*\))',
        'Switch_tab': r'(?s).*?switch_tab\((\d+)(?:\s*\)|\s*,\s*.*\))',
        'User_input': r'(?s).*?user_input\([\'\"](.+)[\'\"](?:\s*\)|\s*,\s*.*\))',
        'Finish': r'(?s).*?finish\((?:[\'\"](.*)[\'\"]|None)?(?:\s*\)|\s*,\s*.*\))',
    }
